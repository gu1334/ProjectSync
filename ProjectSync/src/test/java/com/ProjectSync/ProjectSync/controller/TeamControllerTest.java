...
class TeamControllerTest {
    ...
    void updateTeam_Success() throws Exception {
        // Dado um UpdateTeamDto para atualizar
        UpdateTeamDto updateTeamDto = new UpdateTeamDto();
        updateTeamDto.setName("Updated Team");

        // Simulação de time já existente
        Team updatedTeam = new Team();
        updatedTeam.setId(1);
        updatedTeam.setName("Updated Team");

        when(teamService.upadateTeam(1, updateTeamDto)).thenReturn(updatedTeam);
    ...
    void updateTeam_NotFound() throws Exception {
        // Dado um UpdateTeamDto para atualizar um time inexistente
        UpdateTeamDto updateTeamDto = new UpdateTeamDto();
        updateTeamDto.setName("Updated Team");

        // Simulação de time inexistente
        when(teamService.upadateTeam(1, updateTeamDto)).thenThrow(new ProjectError("Time não encontrado"));
